...
2. A seção da página atual inicia expandida, com o item ativo selecionado.
3. O usuário expande outra seção e clica no item desejado — um clique navega.
4. A nova seção passa a ser a expandida e o item escolhido fica selecionado.
...
- `menu_radio_<perfil>_<seção>` (`session_state`): seleção atual de cada seção.
...
4. Cada seção lembra a última página escolhida durante a sessão.
...
4. Dado usuário na página atual, quando a sidebar renderiza, então o item
   correspondente aparece selecionado na seção ativa.
...
- Critério 3 — teste automatizado: `tests/test_menu_navigation.py`.
- Critérios 1, 2, 4, 5 e 6 — verificação manual em navegador com cada perfil
  (expansores por seção, item ativo selecionado e logout funcional).
