...
- As apostas devem ser efetuadas até o horário programado da corrida dentro do app.
...
- Caso o apostador não aposte na primeira corrida do campeonato, sem base para repetição da aposta, a pontuação será 85% do pior pontuador para esta corrida e o benefício da regra da primeira falta será mantido.
...
- Cada participante possui 15 (quinze) fichas para serem apostadas a cada corrida:
    - A aposta deve conter no mínimo 3 pilotos de equipes diferentes (apostou no Hamilton, não pode apostar também no Russell).
    - Sem limite de ficha por piloto, sendo válido, por exemplo, 13 / 1 / 1, desde que respeitada a regra acima.
    - As corridas Sprint seguem a mesma regra e são consideradas provas válidas.
    - Deve ser indicado o piloto que irá chegar em 11º lugar em todas as provas; em caso de acerto, serão computados 25 pontos extras.
- A pontuação do participante será a multiplicação das fichas apostadas em cada piloto pelo número de pontos que ele obteve na prova (fichas x pontos) mais a pontuação do 11º lugar.
- As apostas serão lançadas na planilha de controle hospedada no OneDrive, sendo o placar atualizado e publicado no grupo e WhatsApp após as corridas.
