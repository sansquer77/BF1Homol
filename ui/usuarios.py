...
from services.admin_operations import (
    master_create_usuario,
    master_delete_usuario,
    master_ensure_financial_schema,
    master_record_user_status,
    master_save_fee,
    master_save_financial,
    master_update_password,
    master_update_usuario,
)
...
def _salvar_pagamentos_temporada(temporada: str, pagamentos: dict[int, bool]) -> None:
    _ensure_gestao_financeira_table()
    master_save_financial(temporada, pagamentos)
...
def _salvar_valor_taxa_temporada(temporada: str, valor_taxa: float) -> None:
    _ensure_gestao_financeira_table()
    master_save_fee(temporada, valor_taxa)
...
def _render_gestao_usuarios_tab(perfil: str):
    ...
    with col1:
        if st.button("Atualizar usuário"):
            ...
            campos = {"nome": novo_nome, "email": novo_email, "perfil": novo_perfil, "status": novo_status}
            if novas_faltas is not None:
                campos["faltas"] = int(novas_faltas)
            if not master_update_usuario(int(user_row["id"]), **campos):
            ...
            if status_anterior != novo_status:
                ...
                master_record_user_status(int(user_row["id"]), novo_status, data_referencia=data_referencia_status)
        ...
        if st.session_state["alterar_senha"]:
            ...
            if st.button("Salvar nova senha"):
                if not nova_senha:
                    ...
                    ok = master_update_password(int(user_row["id"]), nova_hash)
                    if ok:
    ...
    if perfil == "master":
        if st.button("Excluir usuário selecionado"):
            if user_row["perfil"] == "master":
                ...
                master_delete_usuario(int(user_row["id"]))
    ...
    if st.button("Adicionar usuário"):
        if not nome_novo or not email_novo or not senha_novo:
          ...
          sucesso = master_create_usuario(nome_novo, email_novo, senha_novo, perfil_novo, status_novo)
          if sucesso:
