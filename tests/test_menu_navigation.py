...
class MenuNavigationTests(unittest.TestCase):
    ...
    # spec: menu-e-navegacao v1.6 — critério 8
    ...
    def test_grupos_do_menu_sem_retangulo_de_expansor(self):
        source = (ROOT / "main.py").read_text(encoding="utf-8")
        self.assertIn('[data-testid="stSidebar"] [data-testid="stExpander"] > details', source)
        ...
        self.assertIn('"fontWeight": "800"', source)

    def test_estilo_inspirado_no_sistema_financeiro(self):
        source = (ROOT / "main.py").read_text(encoding="utf-8")
        self.assertIn('"fontSize": "14px"', source)
        self.assertIn('"fontSize": "13px"', source)
        self.assertIn('"borderRadius": "6px"', source)
        self.assertIn("var(--glass-bg-hover)", source)
        self.assertIn("summary:hover", source)
