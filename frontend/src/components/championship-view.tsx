...
function BetTable({ rows }: { rows: ChampionshipBetRecord[] }) { return rows.length ? <div className="table-scroll" tabIndex={0}><table className="championship-table"><thead><tr><th>Campeão</th><th>Vice</th><th>Equipe</th><th>Registro</th></tr></thead><tbody>{rows.map((row, index) => <tr key={`${row.user_nome ?? "self"}-${row.bet_time ?? index}`}><td>{row.champion}</td><td>{row.vice}</td><td>{row.team}</td><td>{row.bet_time ?? "—"}</td></tr>)}</tbody></table></div> : <p className="panel-empty">Nenhuma alteração registrada.</p>; }
