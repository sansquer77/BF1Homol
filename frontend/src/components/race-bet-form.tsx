...
export function RaceBetForm() {
  ...
  const eleventhConflicts = Boolean(eleventh && used.has(eleventh));
  ...
  const valid = Boolean(selectedRace?.is_open && allocations.length >= (data?.rules.minimum_drivers ?? 1) && total === data?.rules.total_chips && allocations.every((item) => item.chips > 0 && item.chips <= data!.rules.max_chips_per_driver) && teamsValid && eleventh && !eleventhConflicts);
