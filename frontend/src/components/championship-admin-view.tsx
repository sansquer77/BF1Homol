...
export function ChampionshipAdminView() {
  ...
  return <div className="championship-admin-view admin-catalog-view">
    ...
    {data && !loading ? <>
      ...
      <div className={data.can_bet ? "deadline deadline--open" : "deadline deadline--closed"}><strong>{data.can_bet ? "Apostas abertas" : "Apostas encerradas"}</strong><span>{data.deadline_message}</span>{data.deadline ? <small>{dateTime.format(new Date(data.deadline))}</small> : null}</div>
...
function formatTimestamp(value: string | null) { if (!value) return "—"; const parsed = new Date(value); return Number.isNaN(parsed.getTime()) ? value : dateTime.format(parsed); }
