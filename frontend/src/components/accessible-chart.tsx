...
export function AccessibleChart({ points }: { points: EvolutionPoint[] }) {
  ...
  return <section className="panel chart-panel" id="analises" aria-labelledby="chart-title"><div className="panel__heading"><div><p className="eyebrow">Performance</p><h2 id="chart-title">Evolução na temporada</h2></div></div>{points.length ? <><div role="img" aria-label={description}><ApexChart options={options} series={[{ name: "Pontos acumulados", data: points.map((item) => item.cumulative_points) }]} type="area" height={290} /></div><details className="chart-data"><summary>Ver dados do gráfico em tabela</summary><div className="table-scroll" tabIndex={0}><table><caption className="sr-only">Pontos acumulados por etapa</caption><thead><tr><th scope="col">Etapa</th><th scope="col">Pontos</th></tr></thead><tbody>{points.map((item) => <tr key={item.race_id}><td>{item.race_name}</td><td>{item.cumulative_points}</td></tr>)}</tbody></table></div></details></> : <p className="panel-empty">Ainda não há resultados para formar o gráfico.</p>}</section>;
}
