...
function RaceScores({ data, raceId }: { data: Classification; raceId: string }) { const race = data.races.find((item) => String(item.race_id) === raceId); if (!race) return null; const scores = [...race.scores].sort((a, b) => b.points - a.points || a.participant.localeCompare(b.participant)); return <><p className="race-maximum">Máximo da etapa ({race.race_type}): <strong>{number.format(race.maximum_points)} pontos</strong></p><div className="table-scroll"><table className="analysis-table race-score-table"><colgroup><col className="col-position" /><col className="col-participant" /><col className="col-score" /><col className="col-percentage" /><col className="col-cumulative" /></colgroup><thead><tr><th>Pos.</th><th>Participante</th><th>Pontos na prova</th><th>% do máximo</th><th>Acumulado</th></tr></thead><tbody>{scores.map((score) => <tr key={score.participant}><td>{score.position}</td><td>{score.participant}</td><td>{number.format(score.points)}</td><td><strong>{percentage.format(score.maximum_percentage)}%</strong></td><td>{number.format(score.cumulative_points)}</td></tr>)}</tbody></table></div></>; }
