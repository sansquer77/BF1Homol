...
def create_application_logs_table_if_missing() -> None:
    """Cria o log operacional V4 sem alterar as auditorias legadas."""
    retention_days = max(1, int(os.environ.get("APPLICATION_LOG_RETENTION_DAYS", "30")))
    with get_pool().get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(
            """
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_application_logs_created_at ON application_logs(created_at DESC)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_application_logs_level_created ON application_logs(level, created_at DESC)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_application_logs_request_id ON application_logs(request_id) WHERE request_id IS NOT NULL")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_application_logs_user_created ON application_logs(user_id, created_at DESC) WHERE user_id IS NOT NULL")
        cursor.execute(
            "DELETE FROM application_logs WHERE created_at < CURRENT_TIMESTAMP - (%s * INTERVAL '1 day')",
            (retention_days,),
        )
        conn.commit()
