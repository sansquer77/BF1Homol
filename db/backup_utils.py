...
def _prepare_schema_for_restore() -> None:
    ...
    with db_connect() as conn:
        ...
        # (por exemplo, após mudança de equipe). O ID continua sendo a chave.
        cursor.execute("ALTER TABLE pilotos DROP CONSTRAINT IF EXISTS pilotos_nome_key")
        ...
        for rule_column in (
            ...
            cursor.execute(f"ALTER TABLE regras ADD COLUMN IF NOT EXISTS {rule_column}")
        cursor.execute(
            """
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_login_attempts_email_time ON login_attempts(email, tentativa_em DESC)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_login_attempts_ip_time ON login_attempts(ip_address, tentativa_em DESC)")
...
def _repair_insert_legacy_literals(conn, statement: str, table_name: str) -> str | None:
    ...
    if boolean_cols:
        cols = _extract_insert_columns(repaired_stmt)
        payload = _extract_values_payload(repaired_stmt)
        values = _split_sql_csv(payload) if payload is not None else []
        if cols and len(cols) == len(values):
            boolean_changed = False
            for idx, col in enumerate(cols):
                ...
                if token == "0":
                    values[idx] = "FALSE"
                    boolean_changed = True
                elif token == "1":
                    values[idx] = "TRUE"
                    boolean_changed = True
            if boolean_changed:
                repaired_stmt = re.sub(
                    r"(\bVALUES\s*\().*(\)\s*$)",
                    lambda m: f"{m.group(1)}{', '.join(values)}{m.group(2)}",
                    repaired_stmt,
                    flags=re.IGNORECASE | re.DOTALL,
